...
public class LoginCheckFilter implements Filter {
	...
	public void doFilter(ServletRequest request
		...
		try {
			...
			if(isLoginCheckPath(requestURI)) { // whitelist 가 아닌 uri 만을 검증 
				...
				if( session == null ||
					...
					// items url 을 접근 -> 로그인 페이지가 보여져야 한다.
					httpResponse.sendRedirect("/login?redirectURL="+requestURI);
					return; // 여기가 매우 중요, 미인증 사용자는 다음으로 진행하지 않고 종료 !!!
				}
			}
	...
	// 요청한 uri 가 whitelist 에 포함하고 있는지 확인하는 메서드
	private boolean isLoginCheckPath(String requestURI) {
		/*
		 * PatternMatchUtils : 간단한 패턴 매치를 판별하기 위한 Spring에서 제공하는 유틸이다.
		 */
